...
in {
  ...
  #------------------------------------------------------------------------------
  #  nvim with cheatsheet, customized vimrc and plugins
  #------------------------------------------------------------------------------
  nvimWithConfig = let nvimCustom = nixpkgs.neovim.override {
    configure = {
      customRC =
    ...
    in
    nvimCustom.overrideAttrs (oldAttrs: {
      propagatedBuildInputs = (oldAttrs.propagatedBuildInputs or [ ]) ++ [
        nixpkgs.pkgs.par
      ];

      buildInputs = (oldAttrs.buildInputs or [ ]) ++ [
        nixpkgs.pkgs.powerline-fonts
      ];
    });
